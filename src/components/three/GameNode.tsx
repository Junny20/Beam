...
export default function GameNode({
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      ...
      meshRef.current.position.y = position.y + Math.sin(time * 0.3 + index * 0.5) * 0.15;
    ...
    if (glowRef.current) {
      ...
      const pulse = 1 + Math.sin(time * 2 + index) * 0.15 * (0.3 + glowIntensity * 0.7);
        ...
        <Html distanceFactor={8} transform={false}>
          <div className="bg-bg-darker/95 backdrop-blur-md border border-purple-secondary/40 rounded-xl p-4 min-w-[200px] pointer-events-none shadow-glow">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="font-display font-semibold text-white text-sm">{game.name}</p>
            </div>
            <div className="space-y-1 text-xs text-gray-400">
              <div className="flex justify-between">
                <span>Playtime:</span>
                <span className="text-white">{game.playtime} hrs</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-purple-light">{game.achievements.unlocked}/{game.achievements.total}</span>
              </div>
              {glowIntensity > 0.5 && (
