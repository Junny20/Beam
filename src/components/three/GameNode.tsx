...
export default function GameNode({
  ...
  /* ----------------------------
     VISUAL RADIUS (CLAMPED)
  ----------------------------- */
  const radius = clamp(BASE_RADIUS + rawSize * 0.4, BASE_RADIUS, MAX_RADIUS);

  /* ----------------------------
     STABLE PLANET FEATURES
     (deterministic by id/index)
  ----------------------------- */
  const features = useMemo(() => {
    ...
    const ringBoost = glowIntensity * 0.25; // more active games => more accessories
  ...
  useFrame(({ clock }) => {
    ...
    // float entire planet system together (rings + particles + satellite)
    if (rootRef.current) {
      rootRef.current.position.y =
        Math.sin(t * FLOAT_SPEED + index) * FLOAT_AMPLITUDE;
    }

    // planet rotation + hover/selected scaling
    if (meshRef.current) {
    ...
    // ring motion (subtle, not distracting)
    ...
    // debris belt shimmer
    if (debrisRef.current) debrisRef.current.rotation.z += features.debrisSpeed;

    // particle halo orbits
          ...
          <div className="bg-bg-darker/95 backdrop-blur-md border border-purple-secondary/40 rounded-xl p-3 min-w-[180px] pointer-events-none">
            ...
            <div className="text-xs text-gray-400 space-y-0.5">
              <div className="flex justify-between">
                <span>Playtime</span>
                <span className="text-white">{Math.floor(game.playtime)}h</span>
              </div>
              <div className="flex justify-between">
