...
import { calculateNodeProperties, getFriendsWhoOwnGame, getFriendsWhoPlayedRecently } from '@/data/mockGames';
import { Trophy, Clock, Calendar, Target, Users, Zap, Star, TrendingUp, X } from 'lucide-react';
...
export default function GameDetailPanel({ game, onClose }: GameDetailPanelProps) {
  ...
  const { size, glowIntensity } = game ? calculateNodeProperties(game) : { size: 0, glowIntensity: 0 };
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="font-display text-2xl md:text-3xl font-bold text-white">{game.name}</h2>
        ...
        <div className="p-6 pt-4 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-purple-primary/20 rounded-xl p-4 text-center">
              ...
              <p className="text-2xl font-display font-bold text-purple-light">Top {game.topPercentile}%</p>
          ...
          <div className="bg-gradient-to-r from-yellow-500/20 via-orange-500/20 to-yellow-500/20 border border-yellow-500/30 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-sm font-semibold text-yellow-400">Rarest Achievement</p>
                <p className="text-lg font-display font-bold text-white">{game.achievements.rarest.name}</p>
              </div>
            </div>
            <div className="flex items-center gap-4 text-sm">
              <span className="text-gray-400">Only <span className="text-yellow-400 font-semibold">{game.achievements.rarest.percent}%</span> of players have unlocked this</span>
            </div>
          </div>
          ...
          <div className="bg-purple-dark/30 rounded-xl p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-3">Node Universe Properties</p>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="px-3 py-1.5 rounded-full bg-purple-primary/30 text-xs text-purple-light flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full ${glowIntensity > 0.3 ? 'bg-success animate-pulse' : 'bg-gray-500'}`} />
          ...
          {(friendsWhoOwn.length > 0 || friendsWhoPlayed.length > 0) && (
            <div className="border-t border-purple-secondary/20 pt-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-display font-semibold text-white">Friends</h3>
              </div>
              
              {friendsWhoOwn.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm text-gray-400 mb-2">Own this game:</p>
                  <div className="flex flex-wrap gap-2">
                    {friendsWhoOwn.map((friend) => (
                      <div 
                        key={friend.id}
                        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-dark/50"
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-primary to-purple-secondary flex items-center justify-center text-xs font-bold">
                          {friend.avatar}
                        </div>
                        <span className="text-sm text-white">{friend.name}</span>
                        {friend.status === 'playing' && friend.currentGame === game.name && (
              ...
              {friendsWhoPlayed.length > 0 && (
                <div>
                  <p className="text-sm text-gray-400 mb-2">Played recently:</p>
                  <div className="flex flex-wrap gap-2">
                    {friendsWhoPlayed.map((friend) => (
                      <div 
                        key={friend.id}
                        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-dark/30"
                      >
                        <div className="w-6 h-6 rounded-full bg-purple-primary/50 flex items-center justify-center text-xs font-bold">
                          {friend.avatar}
                        </div>
