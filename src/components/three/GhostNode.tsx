...
export default function GhostNode({
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      ...
      meshRef.current.position.y = position.y + Math.sin(time * 0.2 + index * 0.7) * 0.1;
  ...
  return (
    <group position={position}>
      ...
      {[0, 1, 2, 3].map((i) => (
        ...
        >
          <ringGeometry args={[size * 1.3, size * 1.35, 16, 1, 0, Math.PI / 3]} />
          <meshBasicMaterial
            color={game.color}
            transparent
            opacity={0.4}
        ...
        <Html distanceFactor={8} transform={false}>
          <div className="bg-bg-darker/95 backdrop-blur-md border border-dashed border-gray-500/50 rounded-xl p-4 min-w-[200px] pointer-events-none">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs text-gray-500 uppercase tracking-wider">Discovery</span>
            </div>
            ...
            <div className="mt-3 pt-2 border-t border-gray-700/50">
              <p className="text-xs text-gray-500 italic">Click to learn more</p>
            </div>
          </div>
        </Html>
      )}
